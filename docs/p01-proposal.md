# Movies, Money, and More : Project Proposal 
...
Movies, television, reviews, budget, gross
...
### Background- describe topic, setting
...
Using our data, we will be able to compare differences between movie/TV show budgets, reviews, grosses, and more. This will allow others to see if there is any correlation between said sections. These correlations might allow movie directors or others to determine whether choices regarding budget or other factors will impact the reviews or grosses. Another possible implication includes finding correlations between a movie/TV show’s language, release year, genre, and other subtopics. This can help us find imbalances between reviews or other forms of judging credibility possibly. For example, we might find that one language’s movies/TV shows have unnaturally high or low average reviews, which could lead to more questions about imbalances or other topics. We also might find a correlation between a movie’s length and reviews or grosses which could lead to more questions.

## Expected Limitations

One limitation might include some movies not being included in both movie datasets. To address this limitation, we can manually input movie data to correct the absence. Another limitation might include absences of some movie reviews. Some movies in the dataset do not include reviews, but most do. This can be solved by possibly manually inputting missing reviews. Movie reviews are also hard to gauge because different people/websites have different ways of judging how good a movie is. A small introductory blurb about this is all we can really do about this limitation, but overall, movie reviews are mostly agreeable.


## Acknowledgments

Thank you professor or whoever put the Data Science: Project Design Brief together because it helped with this, especially the links.
...
Are we allowed to manually input missing data?
